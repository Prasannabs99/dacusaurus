"use strict";Object.defineProperty(exports, "__esModule", {value: true});

var _JWSK57DVcjs = require('../__chunks/JWSK57DV.cjs');
require('../__chunks/NEZR6RYD.cjs');


var _P66OGEAYcjs = require('../__chunks/P66OGEAY.cjs');
require('../__chunks/BUKUAI2F.cjs');
require('../__chunks/MIBORXQW.cjs');
require('../__chunks/2P3WACML.cjs');
require('../__chunks/UJOJUNIC.cjs');


var _7BT6R2A7cjs = require('../__chunks/7BT6R2A7.cjs');


var _2JLI27ZJcjs = require('../__chunks/2JLI27ZJ.cjs');
require('../__chunks/7ZBP2RUN.cjs');
require('../__chunks/2XWRXJTW.cjs');
require('../__chunks/V7JASX7S.cjs');
require('../__chunks/FHMITJ2I.cjs');
require('../__chunks/HOK4HLEC.cjs');
require('../__chunks/UGPAREOW.cjs');
require('../__chunks/CWQPJ74P.cjs');
require('../__chunks/ENNAUHNH.cjs');
require('../__chunks/56NPFF7P.cjs');
require('../__chunks/W32FX7DM.cjs');
require('../__chunks/WWQLUY5W.cjs');
require('../__chunks/AQ2LO7WV.cjs');
require('../__chunks/YVOQ2BCB.cjs');
require('../__chunks/4SKDRUBR.cjs');
require('../__chunks/PRCTR2DH.cjs');
require('../__chunks/5UGU5HIQ.cjs');
require('../__chunks/L634CRNJ.cjs');
require('../__chunks/GAIXLTBN.cjs');
require('../__chunks/XQCGTEP2.cjs');
require('../__chunks/UI5KWBYV.cjs');
require('../__chunks/JLMLKTUB.cjs');
require('../__chunks/WFEIS2SR.cjs');
require('../__chunks/SY6ACUYS.cjs');
require('../__chunks/5X5VM34K.cjs');
require('../__chunks/E3JIN4WH.cjs');
require('../__chunks/EPHIWWY3.cjs');
require('../__chunks/XFOTDXSH.cjs');
require('../__chunks/T3QB4FR3.cjs');
require('../__chunks/CSAYKKWD.cjs');




var _MFAH5VLWcjs = require('../__chunks/MFAH5VLW.cjs');
require('../__chunks/2BYHFNPP.cjs');
require('../__chunks/VZG3Q4VA.cjs');
require('../__chunks/Y7UE4JBR.cjs');
require('../__chunks/TM5Y6AJU.cjs');
require('../__chunks/FDRJDQ5Y.cjs');



var _7EQBAZ46cjs = require('../__chunks/7EQBAZ46.cjs');

// src/select/select-popover.tsx
var TagName = "div";
var useSelectPopover = _MFAH5VLWcjs.createHook.call(void 0, 
  function useSelectPopover2(_a) {
    var _b = _a, { store, alwaysVisible } = _b, props = _7EQBAZ46cjs.__objRest.call(void 0, _b, ["store", "alwaysVisible"]);
    const context = _P66OGEAYcjs.useSelectProviderContext.call(void 0, );
    store = store || context;
    props = _JWSK57DVcjs.useSelectList.call(void 0, _7EQBAZ46cjs.__spreadValues.call(void 0, { store, alwaysVisible }, props));
    props = _7BT6R2A7cjs.usePopover.call(void 0, _7EQBAZ46cjs.__spreadValues.call(void 0, { store, alwaysVisible }, props));
    return props;
  }
);
var SelectPopover = _2JLI27ZJcjs.createDialogComponent.call(void 0, 
  _MFAH5VLWcjs.forwardRef.call(void 0, function SelectPopover2(props) {
    const htmlProps = useSelectPopover(props);
    return _MFAH5VLWcjs.createElement.call(void 0, TagName, htmlProps);
  }),
  _P66OGEAYcjs.useSelectProviderContext
);



exports.SelectPopover = SelectPopover; exports.useSelectPopover = useSelectPopover;
